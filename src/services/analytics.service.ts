...
const devLog = _devLog;
const devWarn = _devWarn;
...
function sanitizeEventParams(params: Record<string, unknown>): Record<string, unknown> {
    ...
    if (blockedFound.length > 0) {
        devWarn('[Analytics] Blocked PII params:', blockedFound);
    }
...
function isDoNotTrackEnabled(): boolean {
    ...
    const dnt = navigator.doNotTrack || window.doNotTrack || (navigator as { msDoNotTrack?: string }).msDoNotTrack;
...
export function trackEvent(
    ...
    if (process.env.NODE_ENV !== 'production') {
        devLog('[Analytics Dev]', eventName, safeParams);
        return;
    }
...
// ============================================
// SCAN & PAYMENT EVENT TRACKING
// ============================================

/**
 * Track scan funnel progression
 */
export function trackScanEvent(type: 'scan_started' | 'report_generated' | 'scan_failed', scanId?: string): void {
    trackEvent(type, {
        scan_id: scanId
    });
}
...
/**
 * Track payment/monetization events
 */
export function trackPaymentEvent(type: 'payment_initiated' | 'payment_success' | 'payment_failed', metadata?: Record<string, unknown>): void {
    trackEvent(type, {
        ...metadata
    });
}
